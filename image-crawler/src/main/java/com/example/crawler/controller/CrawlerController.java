...
import com.example.crawler.model.CrawlerCurlRequest;
import com.example.crawler.model.CrawlerRequest;
import com.example.crawler.model.CrawlerResult;
...
public class CrawlerController {
    ...
    public ResponseEntity<CrawlerResult> downloadImages(@Valid @RequestBody CrawlerRequest request) {
        ...
        Path saveDir;
        String savePathStr;
        if (request.getDownloadPath() != null && !request.getDownloadPath().isBlank()) {
            String customPath = request.getDownloadPath().trim();
            saveDir = Paths.get(customPath).normalize();
            savePathStr = saveDir.toString();
            logger.info("[{}] 使用自定义下载路径: {}", taskId, savePathStr);
        } else {
            saveDir = Paths.get(CRAWLER_DIR, timestamp);
            savePathStr = "/uploads/crawler/" + timestamp + "/";
        }
        try {
        ...
        result.setSuccess(true);
        result.setSavePath(savePathStr);
        ...
        logger.info("[{}] 爬虫任务完成，耗时 {}ms，成功: {} 张，失败: {} 张，保存路径: {}",
                taskId, cost, result.getDownloadedCount(), result.getFailedCount(), savePathStr);
    ...
    public ResponseEntity<CrawlerResult> downloadImagesFromCurl(@RequestBody CrawlerCurlRequest curlRequest) {
        ...
        if (curlRequest.getNameField1() != null) {
            request.setNameField1(curlRequest.getNameField1());
        }
        if (curlRequest.getNameField2() != null) {
            request.setNameField2(curlRequest.getNameField2());
        }
        if (curlRequest.getDownloadPath() != null) {
            request.setDownloadPath(curlRequest.getDownloadPath());
        }
    ...
    private Set<String> extractImageUrlsFromJson(String jsonBody, CrawlerRequest request, String taskId, Map<String, String> customNames) {
        ...
        if (targetList != null && !targetList.isEmpty()) {
            ...
            // 调试：打印第一个元素的可用字段名
            if (logger.isDebugEnabled() && !targetList.isEmpty() && targetList.get(0) instanceof Map) {
                Map<?, ?> first = (Map<?, ?>) targetList.get(0);
                logger.debug("[{}] 第一个元素可用字段: {}", taskId, first.keySet());
            }
            ...
            for (Object item : targetList) {
                ...
                if (!urlField.isEmpty()) {
                    urlObj = getNestedValue(itemMap, urlField);
                }
                ...
                if (urlObj == null) {
                    ...
                    for (String f : commonUrlFields) {
                        Object val = getNestedValue(itemMap, f);
                        if (val != null) {
                            urlObj = val;
                ...
                // 如果直接字段没找到，检查是否有 JSON 字符串字段（如 answerSheet）包含 URL
                List<String> urlsFromItem = new ArrayList<>();
                if (urlObj != null) {
                    urlsFromItem.add(urlObj.toString().trim());
                } else {
                    // 遍历所有字段，查找值为 JSON 字符串的字段并解析其中的 URL
                    for (Map.Entry<?, ?> entry : itemMap.entrySet()) {
                        ...
                        if (val instanceof String) {
                            ...
                            if ((strVal.startsWith("[") && strVal.endsWith("]"))
                                ...
                                try {
                                    Object parsed = mapper.readValue(strVal, new TypeReference<Object>() {});
                                    List<String> foundUrls = extractUrlsFromParsedJson(parsed, pattern, urlField);
                                    urlsFromItem.addAll(foundUrls);
                                } catch (Exception e) {
                                    logger.debug("[{}] 字段 '{}' 的值看起来像 JSON 但解析失败: {}", taskId, entry.getKey(), e.getMessage());
                                }
                            }
                        }
                    }
                    if (!urlsFromItem.isEmpty()) {
                        logger.info("[{}] 从 JSON 字符串字段中解析到 {} 个 URL", taskId, urlsFromItem.size());
                    }
                }

                for (String url : urlsFromItem) {
                    if (!url.isBlank() && isValidUrlString(url) && pattern.matcher(url).matches()) {
                        imageUrls.add(url);
                        logger.info("[{}] 从 JSON 提取到 URL: {}", taskId, url);

                        // 提取自定义名称（支持嵌套路径如 user.deptName）
                        if (customNames != null && (!nameField1.isEmpty() || !nameField2.isEmpty())) {
                            StringBuilder customName = new StringBuilder();
                            if (!nameField1.isEmpty()) {
                                Object v1 = getNestedValue(itemMap, nameField1);
                                logger.debug("[{}] 尝试获取 nameField1='{}', 结果={}", taskId, nameField1, v1);
                                if (v1 != null) customName.append(sanitizeFileName(v1.toString()));
                            }
                            if (!nameField2.isEmpty()) {
                                Object v2 = getNestedValue(itemMap, nameField2);
                                logger.debug("[{}] 尝试获取 nameField2='{}', 结果={}", taskId, nameField2, v2);
                                if (v2 != null) {
                            ...
                            if (customName.length() > 0) {
                                ...
                                logger.info("[{}] 生成自定义名称: {} -> {}", taskId, url, customName);
                            } else {
                                logger.warn("[{}] 自定义名称字段 '{}' 和 '{}' 在元素中未找到值，可用字段: {}",
                                        taskId, nameField1, nameField2, itemMap.keySet());
                            }
                        }
                    } else if (!url.isBlank()) {
    ...
    /**
     * 从已解析的 JSON 对象/数组中提取指定字段的 URL 值。
     *
     * @param node      Jackson 解析后的 JSON 节点
     * @param pattern   URL 匹配正则（用于过滤）
     * @param urlField  指定的 URL 字段名，为空时自动探测
     * @return 提取到的 URL 列表
     */
    private List<String> extractUrlsFromParsedJson(Object node, Pattern pattern, String urlField) {
        ...
        if (node instanceof String) {
            ...
            for (Object item : (List<?>) node) {
                urls.addAll(extractUrlsFromParsedJson(item, pattern, urlField));
            }
        } else if (node instanceof Map) {
            Map<?, ?> map = (Map<?, ?>) node;
            // 先尝试指定字段名
            if (urlField != null && !urlField.isBlank()) {
                Object val = map.get(urlField);
                if (val instanceof String) {
                    String s = ((String) val).trim();
                    if (!s.isBlank() && isValidUrlString(s) && pattern.matcher(s).matches()) {
                        urls.add(s);
                        return urls; // 找到了就返回，避免重复
                    }
                }
            }
            // 再尝试常见 URL 字段名
            String[] commonFields = {"url", "imageUrl", "imgUrl", "src", "fileUrl", "downloadUrl"};
            ...
            for (Map.Entry<?, ?> entry : map.entrySet()) {
                urls.addAll(extractUrlsFromParsedJson(entry.getValue(), pattern, urlField));
            }
        }
        return urls;
    }
    ...
    private boolean downloadImage(String imageUrl, Path saveDir, String referer,
        ...
        try {
            ...
            logger.info("图片下载成功: {} -> {}", imageUrl, actualFileName);
            result.addImage(imageUrl);
            result.getFileNames().put(imageUrl, actualFileName);
            return true;
        } catch (Exception e) {
    ...
    /**
     * 从 Map 中按点号路径获取嵌套值（如 "user.deptName"）。
     *
     * @param map  当前层级的 Map
     * @param path 字段路径，支持单层（如 "name"）或多层（如 "user.name"）
     * @return 字段值，不存在返回 null
     */
    private Object getNestedValue(Map<?, ?> map, String path) {
        if (map == null || path == null || path.isBlank()) return null;
        String[] parts = path.split("\\.");
        Object current = map;
        for (String part : parts) {
            part = part.trim();
            if (part.isEmpty() || !(current instanceof Map)) return null;
            current = ((Map<?, ?>) current).get(part);
    ...
    private void saveCallLog(String taskId, long startTime, CrawlerRequest request, CrawlerResult result) {
        ...
        req.put("downloadPath", request.getDownloadPath());
        log.put("request", req);
